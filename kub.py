...
class Kubernetes(BaseModel):
	...
	def get_config(self) -> str:
		return self.run(f'kubectl config view')
	
	def get_context(self) -> str:
		return self.run(f'kubectl config current-context')
		 
	def get_namespace(self) -> str:
		return self.run(f'kubectl get namespace')
	...
	def create_pod(self, path: Path|str|None = None, pod: Pod | None = None) -> str:
		...
		out = self.run(f'kubectl create -f {path}', show= True)
		return out
